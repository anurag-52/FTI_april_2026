...
email_resend.py — Resend transactional email API
...
Rules:
  ...
  - Log every send in notification_log (provider_ref = Resend message ID)
"""
...
def send_email(
    ...
    try:
        ...
        provider_ref = resp.get("id") if isinstance(resp, dict) else getattr(resp, "id", None)
        success = True
        ...
        logger.error(f"Resend exception for {to_email}: {e}")
        if user_id:
            _log_notification(user_id, notification_type, subject,
...
def send_daily_signal(user: dict, session_token: str, buy_count: int, exit_count: int) -> bool:
    ...
    if not user.get("notify_email"): return False
    link = f"{FRONTEND_URL}/confirm/{session_token}"
    name = user.get("full_name", "Trader").split()[0]
    ...
    return send_email(user["email"], f"📊 {total} Signal{'s' if total != 1 else ''} Ready — Confirm Now", _base_html(content, "Daily Signals"), user["id"], "DAILY_SIGNAL")
...
def send_no_signal_day(user: dict) -> bool:
    ...
    return send_email(user["email"], "📊 No Signals Today — All Clear", _base_html(content, "No Signals Today"), user["id"], "NO_SIGNAL_DAY")
...
def send_market_holiday(user: dict, holiday_name: str) -> bool:
    ...
    return send_email(user["email"], f"🏖️ Market Holiday: {holiday_name}", _base_html(content, "Market Holiday"), user["id"], "MARKET_HOLIDAY")
...
def send_reminder(user: dict, session_token: str) -> bool:
    ...
    content = f"""
    <p style="color:#1E293B;font-size:15px;margin:0 0 8px;">Hello {name},</p>
    {_warning_box("⏰", "Pending Confirmation", "Yesterday's signals are still unconfirmed. Please update your confirmations to continue receiving today's signals.")}
    {_cta_button("📋 Confirm Now →", link)}"""

    return send_email(user["email"], "⏰ Reminder — Signals Awaiting Your Confirmation", _base_html(content, "Confirmation Reminder"), user["id"], "REMINDER")
...
def send_inactivity_day5(user: dict, session_token: str) -> bool:
    ...
    content = f"""
    <p style="color:#1E293B;font-size:15px;margin:0 0 8px;">Hello {name},</p>
    {_warning_box("⚠️", "Inactivity Warning — Day 5", "You have not confirmed signals for 5 trading days. Your account will be <strong>auto-paused in 2 days</strong>. Please log in and confirm your actions.")}
    ...
    return send_email(user["email"], "⚠️ Action Required — Day 5 Inactivity Warning", _base_html(content, "Inactivity Warning"), user["id"], "INACTIVITY_DAY5")
...
def send_inactivity_day12(user: dict, session_token: str) -> bool:
    ...
    content = f"""
    <p style="color:#1E293B;font-size:15px;margin:0 0 8px;">Hello {name},</p>
    {_danger_box("🚨", "Critical Warning — Day 12", "Your account is currently paused. It will be <strong>auto-suspended in 3 days</strong> if no action is taken. Once suspended, you will not be able to log in.")}
    {_cta_button("🔑 Log In Now →", link)}
    <p style="color:#DC2626;font-weight:600;font-size:13px;text-align:center;">
      Contact admin at aaanurag@yahoo.com or +91 9303121500 if you need help.
    </p>"""

    return send_email(user["email"], "🚨 URGENT — Account Suspension in 3 Days", _base_html(content, "Suspension Warning"), user["id"], "INACTIVITY_DAY12")
...
def send_auto_suspended(user: dict) -> bool:
    ...
    content = f"""
    <p style="color:#1E293B;font-size:15px;margin:0 0 8px;">Hello {name},</p>
    {_danger_box("🔒", "Account Suspended", "Your account has been suspended due to inactivity (15 trading days with no confirmation). You will no longer be able to log in or receive signals.")}
    ...
    return send_email(user["email"], "🔒 Account Suspended — Contact Admin to Reactivate", _base_html(content, "Account Suspended"), user["id"], "AUTO_SUSPENDED")
...
def send_scan_failure(admin: dict, scan_date: str, attempt: int, source: str, error_msg: str = "") -> bool:
    ...
    if not admin.get("notify_email"): return False
    link = f"{FRONTEND_URL}/admin/system"
    content = f"""
    <p style="color:#1E293B;font-size:15px;margin:0 0 8px;">Hello Admin,</p>
    {_danger_box("🚨", "Daily Scan Failed", f"The daily scan failed at 4:30 PM on {scan_date}.<br>Retry attempt: <strong>{attempt}/12</strong><br>Data source: <strong>{source}</strong>" + (f"<br>Error: <code style='font-size:12px;'>{error_msg[:200]}</code>" if error_msg else ""))}
    ...
    return send_email(admin["email"], f"🚨 URGENT: Daily Scan Failed — Attempt {attempt}/12", _base_html(content, "Scan Failure Alert"), admin["id"], "SCAN_FAILURE")
...
def send_stock_suspended(user: dict, stock_name: str, ticker: str) -> bool:
    ...
    content = f"""
    <p style="color:#1E293B;font-size:15px;margin:0 0 8px;">Hello {name},</p>
    {_warning_box("⚠️", f"Stock Alert: {stock_name} ({ticker})", f"No price data has been received for <strong>{stock_name} ({ticker})</strong> for 3 consecutive trading days. This stock may have been suspended or delisted.")}
    <p style="color:#1E293B;font-size:14px;line-height:1.5;margin:16px 0;">
    ...
    return send_email(user["email"], f"⚠️ Stock Alert: {stock_name} ({ticker}) — No Data for 3 Days", _base_html(content, "Stock Suspension Alert"), user["id"], "STOCK_SUSPENDED")
